import { useEffect, useRef, useState } from "react";
import { Send } from "lucide-react";
import { Input } from "@/components/ui/input";
import { MODES, type ModeKey } from "./types";
import { CHIPS, GREETINGS, getReply, type LangKey } from "./content";

interface Message {
  id: string;
  from: "uthisha" | "user";
  text: string;
}

interface Props {
  mode: ModeKey;
  language: LangKey;
  codeSwitch: boolean;
}

export function ChatPanel({ mode, language, codeSwitch }: Props) {
  const greeting = GREETINGS[codeSwitch ? language : "en"];
  const [messages, setMessages] = useState<Message[]>([
    { id: "greet", from: "uthisha", text: greeting },
  ]);
  const [input, setInput] = useState("");
  const [typing, setTyping] = useState(false);
  const scrollRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    setMessages((prev) =>
      prev.map((m) => (m.id === "greet" ? { ...m, text: greeting } : m)),
    );
  }, [greeting]);

  useEffect(() => {
    scrollRef.current?.scrollTo({ top: scrollRef.current.scrollHeight, behavior: "smooth" });
  }, [messages, typing]);

  const send = (raw: string) => {
    const text = raw.trim();
    if (!text || typing) return;
    const stamp = Date.now();
    setMessages((prev) => [...prev, { id: `u-${stamp}`, from: "user", text }]);
    setInput("");
    setTyping(true);
    window.setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        { id: `a-${stamp}`, from: "uthisha", text: getReply(text) },
      ]);
      setTyping(false);
    }, 700);
  };

  return (
    <div className="space-y-6">
      <header>
        <h1 className="font-display text-2xl font-bold">Uthisha AI Chat Coach</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          {MODES[mode].icon} Conversational tech mentor · {MODES[mode].label}
        </p>
      </header>

      <section className="flex h-[560px] flex-col overflow-hidden rounded-3xl border bg-card shadow-[var(--shadow-soft)]">
        <div className="flex items-center gap-3 border-b bg-secondary/50 px-5 py-4">
          <Avatar />
          <div className="min-w-0">
            <p className="truncate text-sm font-bold">Uthisha</p>
            <p className="truncate text-[11px] text-muted-foreground">
              <span className="mr-1 inline-block h-2 w-2 rounded-full bg-success align-middle" />
              Online · always patient
            </p>
          </div>
        </div>

        <div ref={scrollRef} className="flex-1 space-y-4 overflow-y-auto p-5">
          {messages.map((m) =>
            m.from === "uthisha" ? (
              <div key={m.id} className="flex items-end gap-3">
                <Avatar small />
                <p className="max-w-[85%] whitespace-pre-wrap rounded-3xl rounded-bl-md bg-secondary px-4 py-3 text-sm leading-relaxed">
                  {m.text}
                </p>
              </div>
            ) : (
              <div key={m.id} className="flex justify-end">
                <p className="max-w-[85%] whitespace-pre-wrap rounded-3xl rounded-br-md bg-primary px-4 py-3 text-sm leading-relaxed text-primary-foreground">
                  {m.text}
                </p>
              </div>
            ),
          )}
          {typing && (
            <div className="flex items-end gap-3">
              <Avatar small />
              <p className="rounded-3xl rounded-bl-md bg-secondary px-4 py-3 text-sm text-muted-foreground">
                Uthisha is typing…
              </p>
            </div>
          )}
        </div>

        <div className="border-t p-4">
          <div className="flex gap-2 overflow-x-auto pb-3">
            {CHIPS[mode].map((chip) => (
              <button
                key={chip}
                type="button"
                onClick={() => send(chip)}
                className="shrink-0 rounded-full border border-gold/50 bg-gold/12 px-4 py-2 text-xs font-semibold text-gold-foreground transition-colors hover:bg-gold/20"
              >
                {chip}
              </button>
            ))}
          </div>
          <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-2">
            <Input
              value={input}
              onChange={(e) => setInput(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && send(input)}
              placeholder="Buza Uthisha — ask anything…"
              aria-label="Message Uthisha"
              className="rounded-2xl bg-secondary/60"
            />
            <button
              type="button"
              onClick={() => send(input)}
              aria-label="Send message"
              className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-[image:var(--gradient-gold)] text-gold-foreground shadow-[var(--shadow-glow)]"
            >
              <Send className="h-4 w-4" />
            </button>
          </div>
        </div>
      </section>
    </div>
  );
}

function Avatar({ small }: { small?: boolean }) {
  return (
    <span
      className={
        (small ? "h-8 w-8 text-sm" : "h-10 w-10 text-base") +
        " grid shrink-0 place-items-center rounded-2xl bg-[image:var(--gradient-hero)] text-primary-foreground"
      }
      aria-hidden="true"
    >
      👩🏾‍🏫
    </span>
  );
}
